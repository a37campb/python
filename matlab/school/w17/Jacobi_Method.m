...
for i = 1:n 
    b_val_input = input('Please enter a b-value:    ');
    b_vals(i,1) = b_val_input;
    % Only asking the user for one of the guesses, then sets the remaining
    % x-values to zero.
    if i == 1
        x_vals = input(['Please enter the root-',num2str(i),' guess:  ']);
        x(i,1) = x_vals;
    elseif i > 1
        x(i,1) = 0; 
    end
    for c = 1:n
        a_val_input = input(['Please enter the a-value coresponding to root-',num2str(c),'  ']);
...
for i = 1:n
    ...
    for j = 1:n
        if i ~= j
            summation = summation + a_vals(i,j);
        end
    end
    if summation < a_vals(i,i)
...
if diagonal_count == n % Cheking if diagonally dominant, then the code will run, if not, the error statement will be printed.
    ...
    while m ~= n
        ...
        a = a+1;
        if a == 2 % for first iteration there is no xr, so I have to use x, and then create an xr
            xr_old = x; % Temporarily storing the old x-values
    ...
    disp(' ');
    disp('The matrix entered is not diagonally dominant, and hence the Jacobi Iteration method will diverge.');
    disp(' ');
