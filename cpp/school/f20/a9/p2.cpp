            ...
            for (int j = 0; j < dim[1]; j++) {
                for (int i = 0; i < dim[0]; i++) {
                    if (j == 0) {
                        ...
                        temp[i][j] = D*dt/(pow(dx,2)) * (temp[i+1][j-1] - 2*temp[i][j-1] + temp[dim[0]-1][j-1] ) + temp[i][j-1];
                    } else if (i == (dim[0] - 1)) {
                        //same value using the first boundary
                        temp[i][j] = D*dt/(pow(dx,2)) * (temp[0][j-1] - 2*temp[i][j-1] + temp[i-1][j-1] ) + temp[i][j-1];
                    } else {
...
int main(){
    int tgrid = 10000, xgrid1 = 100, xgrid2 = 10000;
