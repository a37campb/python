...
class DerivativeCalculator{
    ...
    public:
        ...
        double deriv_4(double aX, int n){
            double num, denom;
            num = (*iF)(aX-2*iDx,n)-8*(*iF)(aX - iDx,n) + 8*(*iF)(aX+iDx,n) - (*iF)(aX+2*iDx,n);
            denom = 12*iDx;
            return (num/denom);
        }
...
int main(){
    ...
    double x_val = 1.0;
    int n = 4; //Power to raise x, in function call. 
    double log_dx[10], dx_pow3[10], dx_pow4[10], y_4th[10], error_4th[10];
    for (int i = 0; i < 10; i++){
        y_4th[i] = DC1.deriv_4(x_val, n);
        error_4th[i] = log10(abs(y_4th[i] - 6)); //using n=6 in the error calculation, using n=4 causes floating point error
        log_dx[i] = log10(DC1.dx());
        dx_pow3[i] = pow(DC1.dx(), 3);
        dx_pow4[i] = pow(DC1.dx(),4);
        DC1.setDx( DC1.dx()/1.3);
    }
    DC1.draw(dx_pow3, y_4th, 1);
    DC1.draw(dx_pow4, y_4th, 2);
    DC1.draw(log_dx, error_4th, 0);
    return 0;
}